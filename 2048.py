...
def tilt_field(f, cmd):
    f2=get_new_field()
    for n in range(4):
        if cmd=="i":liste=getnums(f,c=[n])
        if cmd=="j":liste=getnums(f,r=[n])
        if cmd=="k":liste=getnums(f,c=[n])[::-1]
        if cmd=="l":liste=getnums(f,r=[n])[::-1]
        start_len=len(liste)
        while 0 in liste:
            liste.remove(0)
        i=0
        ...
        if cmd=="i": liste=sert(f2,liste, c=[n])
        if cmd=="j": liste=sert(f2,liste, r=[n])
        if cmd=="k": liste=sert(f2,liste, c=[n], reverse = True)
        if cmd=="l": liste=sert(f2,liste, r=[n], reverse = True)
    return f2
...
while not (f==tilt_field(f,"i")==tilt_field(f,"j")
==tilt_field(f,"k")==tilt_field(f,"l")):
    cmd=""
    pprint(f)
    while cmd not in["i","j","k","l"]:
        cmd=raw_input(">")
    f2=tilt_field(f,cmd)
